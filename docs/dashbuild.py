...
sys.path.append(os.path.abspath(os.path.join('..', 'pcstools')))
import version
...
def main(args):
	...
	try:
		db_conn.execute('INSERT INTO "searchIndex" ("name","type","path") VALUES ' \
			...
			'("2 About pcstools", "Guide", "about.html"), ' \
			'("3 Installation", "Guide", "install.html"), ' \
			'("4 Getting Started", "Guide", "intro.html"), ' \
		db_conn.execute('DELETE FROM "searchIndex" WHERE "type" = "Module" AND ("name" = "pcstools")')
		db_conn.commit()
	finally:
		db_conn.close()
